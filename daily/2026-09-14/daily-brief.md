...
**窗口：** 北京时间 2026-09-10 16:00 至 2026-09-14 00:00  
**一句话结论：** 本轮完成 111 个主注册信源的连通性审计，并对 34 个 Feed、Release 与 Sitemap 通道执行增量发现。游标复核发现，采集器标成“新增”的 18 个条目全是此前见过的 OpenAI Sitemap URL；去掉 `lastmod` 抖动后，真实增量为 0。
...
- AWS Security RSS：checked_no_match，检查 20 条最近 feed/release 项。
...
- Google Agent Development Kit releases：candidate_only，检查 10 条最近 feed/release 项。
...
- OpenHands releases：candidate_only，检查 10 条最近 feed/release 项。
- Simon Willison atom：candidate_only，检查 30 条最近 feed/release 项。
- SWE-agent releases：checked_no_match，检查 10 条最近 feed/release 项。

## 覆盖与缺口

- 主注册信源连通性状态：not_checked 53（仅可访问、未解析内容变化）、access_blocked 6、mechanical_failure 52。
- 日期解析探针状态：checked_no_match 14、candidate_only 13。
...
- 只复核采集器标记的 18 个 `new_candidates`，没有重扫滚动候选池。
- 18 个条目全部是 `sitemap_lastmod`，其 canonical URL 在 2026-08-17 至 2026-09-12 的历史发现产物中已经出现过。
- 逐 URL 打开正文 fallback 后，可确认的发布日期均早于本轮窗口或页面没有独立发布日期。GPT-6 Astra 已由稳定事件 `2026-09-03-openai-gpt6-astra-broad-release-monitorability` 收录，不重复建卡。
- 本轮因此按真实增量 0 条处理；Sitemap `lastmod` 变化不再单独触发跨轮新增。

## 今日判断

1. 代表性 feed/release 巡检没有给出足够强的新证据，因此维持高阈值，不重复昨日事件。
2. Sitemap 的 `lastmod` 会因页面维护反复变化；它只能用于发现，不能代替发布日期或正文变化证明。
3. 本轮主要结果是纠正 18 个游标假阳性，并把真实跨轮增量归零。
...
**一句话结论：** 18 个所谓“新增”全部是 Sitemap `lastmod` 抖动；跨历史去重后，真实增量 0 条。

**判断：** 不重复建卡，不为数量降标。

**覆盖：** not_checked 53，access_blocked 6，mechanical_failure 52；日期解析探针 checked_no_match 14。
