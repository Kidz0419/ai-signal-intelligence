...
def main():
    ...
    funnel = json.loads((DIR / "coverage-funnel.json").read_text())
    ...
    funnel_rows = ''.join(
        f'<tr><td>{esc(item["label_cn"])}</td><td>{item["raw_hits"]}</td><td>{item["date_confirmed"]}</td><td>{item["formal_decisions"]}</td><td>{item["net_new_unique_signals"]}</td><td>{item["evidence_upgrades"]}</td><td>{item["excluded_or_deferred"]}</td></tr>'
        for item in funnel["batches"]
    )
...
<section class="panel"><h2>扩源扫描漏斗</h2><p>{esc(funnel["scope_note_cn"])}</p><div class="metrics"><div class="metric"><strong>{funnel["totals"]["raw_hits"]}</strong>扫描命中</div><div class="metric"><strong>{funnel["totals"]["date_confirmed"]}</strong>日期确认</div><div class="metric"><strong>{funnel["totals"]["formal_decisions"]}</strong>正式判定</div><div class="metric"><strong>{funnel["totals"]["net_new_unique_signals"]}</strong>净新增</div><div class="metric"><strong>{funnel["totals"]["evidence_upgrades"]}</strong>证据升级</div><div class="metric"><strong>{funnel["totals"]["excluded_or_deferred"]}</strong>排除/延后</div></div><p class="audit">{esc(funnel["reconciliation"]["equation"])} 去重/合并共 {funnel["totals"]["deduped_or_merged"]} 条。</p><table><thead><tr><th>扫描批次</th><th>命中</th><th>日期确认</th><th>正式判定</th><th>净新增</th><th>证据升级</th><th>排除/延后</th></tr></thead><tbody>{funnel_rows}</tbody></table></section>
<section class="panel"><h2>信源注册表状态</h2><p>“未检查、访问受限、机械失败”不等于没有新增；“候选”也不计为正式 Signal。</p><table><thead><tr><th>通道</th><th>注册</th><th>入选</th><th>候选</th><th>已查无合格</th><th>受限/失败</th><th>未检查</th></tr></thead><tbody>{coverage_rows}</tbody></table></section>
